...
export default function StockInPage() {
    const formatDisplayDate = (dateStr) => {
        ...
        try {
            ...
            const yr2 = y ? y.slice(-2) : '';
            const dayNum = parseInt(d, 10);
            const monthNum = parseInt(m, 10);

            return `${dayNum}-${monthNum}-${yr2} | ${String(hour).padStart(2, '0')}:${minStr} ${period}`;
        } catch (e) {
            return dateStr;
        }
    };
    ...
    useEffect(() => {
        const handleHideShowOutsideClick = (event) => {
            if (hideShowDropdownRef.current && !hideShowDropdownRef.current.contains(event.target)) {
                setIsHideShowDropdownOpen(false);
            }
        };
        document.addEventListener('mousedown', handleHideShowOutsideClick);
        return () => document.removeEventListener('mousedown', handleHideShowOutsideClick);
    }, []);
    ...
    const logCategoryOptions = logCategories.map(c => ({ value: c._id, label: c.name }));
    ...
    const types = logCategories.length > 0 ? logCategories.map(c => c.name) : ['PURCHASE', 'OPENING', 'RETURN', 'ADJUSTMENT'];
    ...
    const [masterForm, setMasterForm] = useState({
        vendor: '',
        subType: '', // Will be set after logCategories load
        narration: '',
        billPath: '',
        billName: '',
        date: (() => {
    ...
    useEffect(() => {
        if (logCategories.length > 0 && !masterForm.subType) {
            const purchaseCat = logCategories.find(c => c.name.toUpperCase() === 'PURCHASE') || logCategories[0];
            if (purchaseCat) setMasterForm(prev => ({ ...prev, subType: purchaseCat._id }));
        }
    }, [logCategories]);
    ...
    const fetchTransactions = useCallback(async () => {
        try {
            ...
            const res = await fetch(`/api/kitchen/transactions?type=IN&section=general${query}`);
            if (res.ok) {
                const data = await res.json();
    ...
    const loadData = useCallback(() => {
        ...
        const generalQuery = companyId ? `?companyId=${companyId}&section=general` : '?section=general';

        fetch(`/api/kitchen/items${generalQuery}`)
            .then(res => res.json())
            .then(data => {
        ...
        fetch(`/api/categories${generalQuery}`)
            .then(res => res.json())
            .then(data => {
    ...
    const recalculateRow = (row, field) => {
        ...
        const discount = Number(updatedRow.discount) || 0;
        const gstRate = Number(updatedRow.gstRate) || 0;
        const isMrpLinked = updatedRow.isMrpLinked !== false;

        const currentMrp = Number(updatedRow.mrp) || 0;
        const currentTotalAmt = Number(updatedRow.totalAmt) || 0;
        const currentGrandTotal = Number(updatedRow.grandTotal) || 0;

        if (field === 'mrp') {
            const grandTotal = currentMrp * qty;
            updatedRow.grandTotal = formatMoneyValue(grandTotal);
            const totalAmt = (grandTotal + discount) / (1 + gstRate / 100);
            updatedRow.totalAmt = formatMoneyValue(totalAmt);
            updatedRow.gstAmount = formatMoneyValue(totalAmt * gstRate / 100);
            if (isMrpLinked) updatedRow.rate = updatedRow.mrp;
        } else if (field === 'grandTotal') {
            if (qty > 0) {
                updatedRow.mrp = formatMoneyValue(currentGrandTotal / qty);
                if (isMrpLinked) updatedRow.rate = updatedRow.mrp;
            }
            ...
            updatedRow.gstAmount = formatMoneyValue(gstAmount);
            const grandTotal = currentTotalAmt + gstAmount - discount;
            updatedRow.grandTotal = formatMoneyValue(grandTotal);
            if (qty > 0) {
                updatedRow.mrp = formatMoneyValue(grandTotal / qty);
                if (isMrpLinked) updatedRow.rate = updatedRow.mrp;
            }
        ...
        setItemsList(prev => prev.map(row => {
            if (row.tempId === tempId) {
                ...
                if (field === 'item') {
                    ...
                    if (selected) {
                        updatedRow.itemName = selected.name;
                        updatedRow.itemUnit = selected.unit?.name || selected.unit;
                        updatedRow.category = selected.category?._id || selected.category;
                        if (editingId) {
                            if (!updatedRow.gstRate) {
                                updatedRow.gstRate = selected.gst || '';
                            }
                            if (!updatedRow.mrp) {
                                updatedRow.mrp = selected.mrp ? formatMoneyValue(selected.mrp) : '';
                            }
                            if (!updatedRow.rate && updatedRow.isMrpLinked !== false) {
                                updatedRow.rate = selected.mrp ? formatMoneyValue(selected.mrp) : '';
                            }
                        } else {
                            updatedRow.gstRate = selected.gst || '';
                            updatedRow.mrp = selected.mrp ? formatMoneyValue(selected.mrp) : '';
                            if (updatedRow.isMrpLinked !== false) {
                                updatedRow.rate = selected.mrp ? formatMoneyValue(selected.mrp) : '';
                            }
                        }
                    }
                ...
                if (field === 'isMrpLinked') {
                    updatedRow.isMrpLinked = Boolean(value);
                }

                return recalculateRow(updatedRow, field);
            }
            return row;
        }));
    };
    ...
    const handleBulkSubmit = async () => {
        ...
        try {
            ...
            for (const item of finalItems) {
                ...
                // remove direct-out helper props
                ...
                if (item.isDirectOut && newBatchId) {
                    const deptObj = departments.find(d => d.name === item.outDept || String(d._id) === String(item.outDept));
                    const eventObj = events.find(e => e.name === item.outEvent || String(e._id) === String(item.outEvent));

                    let resolvedDept = deptObj ? deptObj._id : item.outDept;
                    if (resolvedDept && String(resolvedDept).length !== 24) resolvedDept = undefined;

                    let resolvedEvent = eventObj ? eventObj._id : item.outEvent;
                    if (resolvedEvent && String(resolvedEvent).length !== 24) resolvedEvent = undefined;
    ...
    const handleEdit = (tx) => {
        ...
        setMasterForm({
            vendor: tx.vendor || '',
            subType: tx.subType?.name || tx.subType || '',
        ...
        const baseRow = {
            ...
            rate: tx.rate,
            gstRate: tx.gstRate ?? tx.gstPercent ?? (selectedItem ? selectedItem.gst : 0),
        ...
        setItemsList([recalculateRow(baseRow, 'item')]);
        setIsModalOpen(false);
    };
    ...
    const handleBulkExport = () => {
        ...
        XLSX.writeFile(wb, `${selectedIds.length > 0 ? 'Selected_' : ''}Stock_In_Report_${new Date().toISOString().slice(0, 10)}.xlsx`);
    ...
    const handleBulkExportPDF = async () => {
        ...
        const headers = ['Date', 'Item', 'Category', 'Quantity', 'Rate', 'Grand Total', 'MRP', 'Narration'];
        const dataToExport = listToExport.map(tx => [
        ...
        const title = selectedIds.length > 0 ? "Selected Stock In" : "Stock In Detail";
        const fileName = `${selectedIds.length > 0 ? 'Selected_' : ''}Stock_In_Report_${new Date().toISOString().slice(0, 10)}.pdf`;
    ...
    // const uniqueItems = [...new Set(transactions.map(t => t.item?.name).filter(Boolean))].sort();
    ...
    // const uniqueGTotals = [...new Set(transactions.map(t => String(Math.round((Number(t.totalAmount) || 0) + (Number(t.gstAmount) || 0)))))].sort((a, b) => Number(a) - Number(b));
    ...
    const uniqueGstRates = [...new Set(transactions.map(t => String(t.gstRate || t.gstPercent || 0)))].sort((a, b) => Number(a) - Number(b));
    const uniqueTotals = [...new Set(transactions.map(t => String(t.totalAmount || 0)))].sort((a, b) => Number(a) - Number(b));
    const uniqueDiscounts = [...new Set(transactions.map(t => String(t.discountAmount || t.discount || 0)))].sort((a, b) => Number(a) - Number(b));
    ...
    const filteredTransactions = transactions.filter(t => {
        ...
        const typeMatch = filterType.length === 0 || filterType.includes(t.type) || filterType.includes(t.subType);
        ...
        if (colFilters.discount?.length && !colFilters.discount.includes(String(t.discountAmount || t.discount || 0))) colsMatch = false;
    ...
    if (permsLoading) return <div className="p-8 text-center text-slate-800">Loading permissions...</div>;
    ...
    return (
        <div className="min-h-screen text-foreground">
            ...
            <main className="p-4 md:p-10 mb-20 md:mb-0 ">
                ...
                <div className="flex justify-end items-center gap-4 mb-4 relative">
                    ...
                    <div ref={hideShowDropdownRef} className="relative">
                        <button
                            ...
                            className="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-full text-xs font-bold text-slate-500 hover:text-slate-700 dark:hover:text-zinc-200 transition-colors shadow-sm cursor-pointer"
                        >
                ...
                />
                <AnimatePresence>
                    {isEntrySectionOpen && (hasPermission('write') || editingId) && (
                            ...
                            {!isReadOnly && (hasPermission('write') || editingId) && (
                                <div className="bg-card rounded-lg shadow-xl shadow-2xl mb-8 overflow-hidden">
                                    <div className="p-6 md:p-8">
                                        ...
                                        {/* Header Part - Styled Tabs Navigation like Image 1 */}
                                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-[#F2E6E1] dark:border-zinc-800  gap-4">
                                            <div>
                                            ...
                                            <div className="flex items-center gap-6 self-end md:self-auto">
                                                ...
                                                <PermissionWrapper action="source">
                                                    <button
                                                        onClick={() => setIsMasterManagerOpen(true)}
                                                        className="flex flex-col items-center gap-1 group text-slate-500 mt-2 hover:text-[#882619] dark:text-white transition-all cursor-pointer bg-transparent border-0 outline-none animate-none"
                                                    >
                                        ...
                                        {/* Master Fields */}
                                        <div className="flex flex-col md:flex-row gap-6 items-center mb-8 bg-[#EBEBEB] dark:bg-zinc-800/60 rounded-none border-t border-b border-[#A4A4A4] dark:border-zinc-800 py-4 px-6 relative w-full overflow-hidden" style={{ '--primary': '#882619' }}>
                                                ...
                                                <DateTimePicker
                                                    ...
                                                    customTrigger={
                                                        <div className="flex items-center justify-between px-4 py-2 bg-white dark:bg-zinc-800 border border-[#D4612D]/40 dark:border-zinc-700 rounded-xl text-xs font-semibold text-[#882619] cursor-pointer select-none w-full h-[38px] shadow-sm hover:border-[#882619] transition-all">
                                                            ...
                                                            <img src="/icons/action/CalenderDark.svg" className="w-4 h-4 transition-transform group-hover:scale-110 hidden dark:block" alt="View Image" />                                                        </div>
                                                    }
                                                />
                                            </div>
                                            <div className="flex-1 w-full">
                                            ...
                                            <table className="w-full text-left whitespace-nowrap border-separate border-spacing-0 min-w-[1250px] bg-white dark:bg-zinc-900">
                                                ...
                                                <tbody className="text-xs font-bold text-foreground">
                                                    {itemsList.map((item, idx) => {
                                                        ...
                                                        const hasData = item.item || item.quantity || item.rate || item.totalAmt;
                                                        const isQtyValid = Number(item.quantity) > 0;
                                                        const isRateValid = Number(item.rate) > 0;
                                                        const isAmtValid = Number(item.grandTotal || item.totalAmt) > 0;
                                                        const isValid = item.item && isQtyValid && isRateValid && isAmtValid;
                                                        ...
                                                        return (
                                                            <tr key={item.tempId} className="bg-white dark:bg-zinc-900 group hover:bg-muted/30">
                                                                ...
                                                                {/* Column 2: Product Name (with To & Event dropdowns nested below it if stock out is active) */}
                                                                <td className={`py-4 px-3 border-r-2 border-[#D4612D] gradient-border-r align-top pl-4 ${idx === itemsList.length - 1 ? 'last-row-double-border' : 'border-b border-[#A4A4A4]'}`}>
                                                                    <div className="flex flex-col min-w-[220px]">
                                                                        <div className="w-full">
                                                                            <SearchableSelect
                                                                                options={items.filter(i => !EXCLUDED_CATEGORIES.includes(i.category?.name || i.category)).map(i => ({ value: i._id, label: i.name }))}
                                                                                value={item.item || ''}
                                                                ...
                                                                {/* Column 3: Qty (with outQty and stock label nested below it if stock out is active) */}
                                                                ...
                                                                <td className={`py-4 px-3 text-center align-middle ${idx === itemsList.length - 1 ? 'last-row-double-border' : 'border-b border-[#A4A4A4]'}`}>
                                                                    <div className="flex justify-center items-center gap-3">
                                                                        {item.item ? (
                                                                            <span className="text-emerald-600 font-extrabold text-lg" title="Valid Entry">                                                            <Check size={18} className="text-emerald-600 font-bold" strokeWidth={3} />
                                                                            </span>
                                                                        ) : (
                                                                            <span className="text-red-500 font-extrabold text-lg" title="Invalid Entry: Check required fields">                                                            <X size={18} className="text-red-500 font-bold" strokeWidth={3} />
                                                                            </span>
                                                                        )}
                                                                        <button
                                                                            type="button"
                                        ...
                                        <div className="gradient-border bg-[#F2F2F2] dark:bg-zinc-800/30 py-6 mt-4 flex flex-col items-center justify-center gap-3">                                            <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 text-[17px] font-bold text-[#7C7470] dark:text-slate-400">
                                            <span>
                                                Total :{' '}
                    ...
                    {!isEntrySectionOpen && (
                        <>
                            <div className="bg-card rounded-xl shadow-2xl shadow-primary/5 overflow-hidden">
                                {/* Header for Table Area - Styled Tabs Navigation like Image 2 */}
                                <div className="p-6 md:p-8 border-t border-[#D4612D] border-b border-[#D4612D] bg-[#E5E5E5] dark:bg-[#222222] dark:border-zinc-800 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                                    ...
                                    {/* Search Input inline next to buttons */}
                                    <div className="relative group w-48 sm:w-64">
                                        ...
                                        <input
                                            ...
                                            className="w-full pl-9 pr-4 py-2 bg-white dark:bg-zinc-800 border border-[#D4612D]/40 rounded-xl text-xs font-bold text-foreground outline-none focus:border-[#D4612D] focus:ring-2 focus:ring-[#D4612D]/20 transition-all placeholder:text-muted-foreground/30 shadow-sm"
                                        />
                                    </div>

                                    <div className="flex flex-wrap items-center gap-6 self-end lg:self-auto relative">
                                        {!isReadOnly && (
                                            <button
                                                onClick={handleBulkDelete}
                                                disabled={selectedIds.length === 0}
                                                className="flex flex-col items-center gap-1 group text-slate-500  transition-all cursor-pointer bg-transparent border-0 outline-none animate-none"
                                            >
                                        ...
                                        <div className="relative" ref={downloadRef}>
                                            <button
                                                ...
                                                className="flex flex-col items-center gap-1 group text-slate-500 hover:text-primary transition-all cursor-pointer bg-transparent border-0 outline-none animate-none"
                                            >
                                        ...
                                        <button
                                            ...
                                            className="flex flex-col items-center gap-1 group text-slate-500 hover:text-primary dark:text-white transition-all cursor-pointer bg-transparent border-0 outline-none animate-none"
                                        >
                                                                            ...
                                                                            <div className="flex items-center gap-3">
                                                                                ...
                                                                                {isEditing ? (
                                                                                    <div className="flex flex-col gap-2 flex-1 min-w-0">
                                                                                        <SearchableSelect
                                                                                            options={items.filter(i => !EXCLUDED_CATEGORIES.includes(i.category?.name || i.category) || String(i._id) === String(editingRow?.item)).map(i => ({ value: i._id, label: i.name }))}
                        ...
                        {isModalOpen && (
                            <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
                                ...
                                >
                                    <div className="flex-1 overflow-y-auto no-scrollbar">
                                        <div className="p-6 md:p-8 bg-muted/30 border-b border-border">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <input
                                                        type="file"
                                                        className="w-full p-2 rounded text-sm bg-muted/50 border border-border text-foreground"
                                                        onChange={handleFileUpload}
                                                        accept="image/*,.pdf"
                                                    />
                                                    {uploading && <p className="text-primary text-xs font-bold absolute -bottom-4">Uploading...</p>}
                                                    {masterForm.billPath && <p className="text-emerald-500 text-xs font-bold absolute -bottom-4">Uploaded</p>}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="p-6 md:p-8 space-y-6">
                                                        ...
                                                        >
                                                            ...
                                                            {items.map(item => (
                                                                <option key={item._id} value={item._id}>
                                                                    {item.name} ({item.unit?.name || item.unit || item.category?.name}) {item.companyId?.name ? `- ${item.companyId.name}` : ''} - GST: {item.gst || 0}%
                                                                </option>
                                                            ))}
                                                        </select>
                                            ...
                                            <button
                                                onClick={handleUpdateSingle}
                                                disabled={uploading}
                                                className="w-full bg-primary text-primary-foreground py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 transition-all shadow-xl shadow-primary/20 hover:bg-primary/90 active:scale-95 disabled:"
                                            >
                                    ...
                                    <button
                                        onClick={() => setIsModalOpen(false)}
                                        className="absolute top-4 right-4 z-[110] text-muted-foreground hover:text-foreground transition-colors p-2 bg-muted/50 backdrop-blur rounded-full"
                                    >
                                        <X size={24} />
                                    </button>
                                </motion.div>
                                            ...
                                            <div>
                                                ...
                                                {viewItem.billPath ? (
                                                    <a
                                                        ...
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        className="flex items-center gap-4 p-5 bg-primary/10 text-primary rounded-2xl hover:bg-primary/20 transition-all group border border-primary/20 shadow-sm"
                                                    >
                                                        ...
                                                        <div>
                                                            ...
                                                            <span className="text-[10px] font-bold text-primary/70 lowercase mt-0.5 block  truncate max-w-[200px]">{viewItem.billPath.split('/').pop()}</span>
                                                        </div>
                                                    </a>
                                                ) : (
                                                    <div className="flex items-center gap-4 p-5 bg-muted text-muted-foreground rounded-2xl border border-border grayscale italic border-dashed">
    ...
    return (
        <div className="space-y-1 md:space-y-1">
            ...
            <div className="relative flex items-center">
                {iconPrefix && <span className="absolute left-4 font-semibold text-muted-foreground/70 pointer-events-none">{iconPrefix}</span>}
                <input
                    {...props}
                    className={`w-full ${iconPrefix ? 'pl-8 pr-4 md:pl-10 md:pr-5' : 'px-4 md:px-5'} ${iconSuffix ? '!pr-8 md:!pr-10' : ''} py-2.5 md:py-3.5 bg-muted/50 border border-border rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all font-bold text-foreground text-sm ${className}`}
                />
                {iconSuffix && <span className="absolute right-4 font-semibold text-muted-foreground/70 pointer-events-none">{iconSuffix}</span>}
            </div>
        </div>
    );
}
...
function CustomSelect({ label, options, ...props }) {
    return (
        <div className="space-y-1.5 md:space-y-2">
            <label className="text-[10px] md:text-xs font-bold text-muted-foreground uppercase tracking-widest ml-1">{label}</label>
            <div className="relative">
                <select {...props} className="w-full appearance-none px-4 md:px-5 py-2.5 md:py-3.5 bg-muted/50 border border-border rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all font-bold text-foreground cursor-pointer text-sm">
                    {[...new Set(options)].map(opt => <option key={opt} value={opt}>{opt}</option>)}
                </select>
                <ChevronDown size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none" />
            </div>
        </div>
