...
export default function VegFruitStockInPage() {
    ...
    const fetchTransactions = React.useCallback(async () => {
        try {
            ...
            const res = await fetch(`/api/kitchen/transactions?type=IN&section=veg-fruits${query}`);
            if (res.ok) {
                const data = await res.json();
    ...
    useEffect(() => {
        ...
        const query = companyId ? `?companyId=${companyId}` : '';
        fetch(`/api/kitchen/items?section=veg-fruits${query ? query.replace('?', '&') : ''}`).then(res => res.json()).then(setItems);
    ...
    const handleRowChange = (tempId, field, value) => {
        setItemsList(prevList => prevList.map(row => {
            ...
            if (field === 'item') {
                ...
                updated.itemUnit = selectedItem ? selectedItem.unit : '';
    ...
    const handleEdit = (tx) => {
        ...
        const baseRow = {
            ...
            itemUnit: selectedItem ? selectedItem.unit : '',
    ...
    const handleBulkExport = () => {
        ...
        XLSX.writeFile(wb, `${selectedIds.length > 0 ? 'Selected_' : ''}VegFruit_In_Report_${new Date().toISOString().slice(0, 10)}.xlsx`);
    };
    ...
    const handleBulkExportPDF = async () => {
        ...
        const title = selectedIds.length > 0 ? "Selected Stock In Report" : "Vegetables & Fruits Inflow Report";
        const headers = ['Date', 'Item', 'Category', 'Quantity', 'Rate', 'Grand Total', 'Narration'];
        const fileName = `${selectedIds.length > 0 ? 'Selected_' : ''}VegFruit_In_Report_${new Date().toISOString().slice(0, 10)}.pdf`;
                        ...
                        >
                            <div className="bg-card rounded-lg shadow-2xl overflow-hidden">
                                <div className="p-8">
                                    ...
                                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-[#F2E6E1] dark:border-zinc-800 gap-4 mb-6 pb-4">
                                        <div>
                                            <h1 className="text-3xl font-extrabold tracking-tight">
                                                <span className="text-[#882619] dark:text-white">Vegetable & Fruit</span>{' '}
                                        ...
                                        <div className="flex items-center gap-6 self-end md:self-auto">
                                            ...
                                            <PermissionWrapper action="source">
                                                <button
                                                    onClick={() => setIsMasterManagerOpen(true)}
                                                    className="flex flex-col items-center gap-1 mt-2 group text-slate-500 hover:text-[#882619] dark:text-white transition-all cursor-pointer bg-transparent border-0 outline-none animate-none"
                                                >
                                    ...
                                    {/* Master Fields */}
                                    <div className="flex flex-col md:flex-row gap-6 items-center mb-8 bg-[#ECEAE6]/40 dark:bg-zinc-800/60 rounded-none border-t border-b border-[#A4A4A4] dark:border-zinc-800 py-4 px-6 relative w-full overflow-hidden" style={{ '--primary': '#882619' }}>
                                    ...
                                    <div className="gradient-border bg-[#EBEBEB] dark:bg-zinc-800/30 py-6 mt-4 flex flex-col items-center justify-center gap-3">
